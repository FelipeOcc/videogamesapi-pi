...
function rootReducer(state= initialState, action){
    switch(action.type) {
        ...
        case 'FILTER_CREATED':
            const filterCreated = action.payload === 'Creado' ? 
            state.allVideogames.filter(createFilter => createFilter.createdInDb) 
            : state.allVideogames.filter( createFilter => !createFilter.createdInDb)
            return {
                ...state,
                videogames: action.payload === 'Todos'
                ? state.allVideogames 
                : filterCreated  
            };
        ...
        let sortRating = action.payload === 'Peor calificado' 
        ?state.videogames.sort(function(a, b) {
